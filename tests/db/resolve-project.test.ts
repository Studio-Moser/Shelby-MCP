...
describe("resolveProjectScope", () => {
	...
	it("resolves a git worktree from the common repository config", () => {
		...
		mkdirSync(join(common, ".git", "worktrees", "feature"), {
			recursive: true,
		});
		writeFileSync(
			join(common, ".git", "config"),
			'[remote "origin"]\n\turl = git@github.com:acme/worktree.git\n',
		);
		const worktree = mkdtempSync(join(tmpdir(), "rp-worktree-"));
		writeFileSync(
			join(worktree, ".git"),
			`gitdir: ${join(common, ".git", "worktrees", "feature")}\n`,
		);

		expect(resolveProjectScope(db, [worktree])).toMatchObject({
			kind: "resolved",
			slug: "worktree",
			source: "derived",
		});
