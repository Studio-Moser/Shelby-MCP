...
function loadFixture(): Fixture {
	try {
		...
		throw new Error(`Invalid project-identity-v2 fixture: ${String(error)}`, {
			cause: error,
		});
	}
}
...
describe("project identity v2 primitive fixture", () => {
	it("derives exact legacy project UUIDv5 values", () => {
		for (const vector of fixture.uuid_v5.vectors) {
			expect(deriveExistingProjectId(vector.legacy_slug)).toBe(
				vector.project_id,
			);
		}
	});
	...
	it("derives exact SHA-256 slug claim record names", () => {
		for (const { slug, record_name: expectedRecordName } of fixture
			.claim_record_names.vectors) {
